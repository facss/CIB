...
class ModelTrain(object):
    ...
    def mlptrain(self,best_val):
        ...
        for epoch in range(self.opts.MLP_Start_epoch,self.opts.MLP_train_number_epochs):       
            ...
            if (epoch+1)%opts.valfre ==0 and epoch !=0:#save the model
                ...
                save_checkpoint({
                    'epoch':epoch+1,
                    'MLPstate_dict':self.MLPModel.state_dict(),
                    'VAEstate_dict':self.VAEModel.state_dict(),
                    'best_val':best_val,
                    'MLPOptimizer':self.MLPOptimizer,
                    'VAEOptimer':self.VAEOptimizer,
                    'curr_val':val_res,
                },is_best)
    ...
    def VAEtrain_epoch(self,epoch,VAEcount):
        #switch to train model
        self.VAEModel.train()
